...
var vm =new Vue({
    ...
    template: '<div id="drawDiv" @mousemove="moveMouse($event)" style="height: 100%;">' +
        ...
        '              <div v-for="(val1, m) in val.fieldMap" style="color:black; border-top: 1px solid #cccccc;display: flex;padding: 0 0.8em; justify-content: space-between" :id="point + \'-\' + m"><div class="param-name" style="font-size: 12px">{{val1.name}}</div></div>' +
        '            </div>' +
    ...
    methods:{
        ...
        calPosition(point, val) {
            if (val.x && val.y) {
                ...
                let randomX = Math.ceil(Math.random()*1000);
                let randomY = Math.ceil(Math.random()*500)
        ...
        createFlow(){
            ...
            vm.instance.batch(() => {
                ...
                const arrowCommon = { foldback: 0.9, width: 10 };
